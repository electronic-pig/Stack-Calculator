...
void button(int x, int y, int w, int h, int textSize, TCHAR* text){
	...
	setfillcolor(0x878A80);//冷灰色
	fillroundrect(x, y, x + w, y + h, 23, 23);//画出圆角矩形
	settextstyle(textSize, 0, "幼圆");//设置字体格式
	//使文本居中对齐
...
void menu_init() {
	...
	TCHAR s1[50] = "从键盘输入";
	TCHAR s2[50] = "从文件输入";
	TCHAR s3[100] = "注：从键盘输入采用中缀表达式直接求值,从文件输入采用中缀转后缀表达式求值";
	//画包含文本的按钮
...
void calcul_memu() {
	...
	setbkmode(TRANSPARENT);//设置文字的背景透明
	settextstyle(60, 35, "幼圆", 0, 0, 750, false, false, false);
...
void mouse_info() {
	...
	while (true) {
		...
		settextstyle(60, 20, "黑体", 0, 0, 0, false, false, false);
		settextcolor(0x46170B);
